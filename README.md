...
### 1. 🔬 微观视角 (Micro View) - **v1.1.0 新升级**
*   **双轨同步演示**：同时追踪主虹（上方入射）和副虹（下方入射）的光路，直观对比两者在水滴内部的不同命运。
*   **视觉差异化**：
    *   **实线**：代表主虹光路（一次反射）。
    *   **虚线**：代表副虹光路（两次反射），贯穿全程，降低认知负担。
*   **高保真光谱**：基于6种不同折射率（红/橙/黄/绿/蓝/紫）渲染更真实的彩虹色带，而非简单的三色线。
*   **交互式水滴模拟**：拖动滑块改变光线射入水滴的位置。
*   **颜色反转演示**：清晰展示副虹为何是“内红外紫”，与主虹相反。
